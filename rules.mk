...
# The documentation site's stylesheet and search script, embedded into the
# core tree the same way the web assets go into the server tree, because
# `venturectl docs build` writes them beside the pages and an installed
# CLI has no data/static to read them from.
DOCS_ASSET_FILES := data/static/venture-docs.css data/static/venture-docs.js

$(OUTDIR)/venture-docs-assets.h: $(DOCS_ASSET_FILES) | $(OUTDIR)
	@echo "  GEN     $@"
	$(Q)echo "/* Generated by rules.mk - do not edit. */" > $@
	$(Q)set -e; for f in $(DOCS_ASSET_FILES); do \
		sym=$$(basename "$$f" | tr '.-' '__'); \
		echo "static const gchar *venture_docs_asset_$$sym =" >> $@; \
		sed 's/\\/\\\\/g; s/"/\\"/g; s/^/"/; s/$$/\\n"/' "$$f" >> $@; \
		echo ";" >> $@; \
		echo "" >> $@; \
	done
