...
upgrade(Fixture *f, gconstpointer data)
{
	...
	g_assert_true(venture_database_execute(f->db,
		"ALTER TABLE sales DROP COLUMN refunded_at; DELETE FROM schema_migrations WHERE version >= 60; DROP TABLE IF EXISTS venture_pipeline_upgrade", NULL, &error));
	g_assert_no_error(error);
	for (i = 0; i < 2; i++) {
		g_assert_true(venture_database_migrate(f->db, venture_entity_registry_get_default(), &error));
		g_assert_no_error(error);
...
module_off(Fixture *f, gconstpointer data)
{
	...
	g_assert_true(venture_database_execute(f->db, "DROP TABLE posting_profiles; DELETE FROM schema_migrations WHERE version >= 60; DROP TABLE IF EXISTS venture_pipeline_upgrade", NULL, &error));
	g_assert_true(venture_database_migrate(f->db, venture_entity_registry_get_default(), &error));
	g_assert_no_error(error);
...
upgrade_disabled(Fixture *f, gconstpointer data)
{
	...
	g_assert_true(venture_database_execute(f->db,
		"DROP TABLE posting_profiles; ALTER TABLE sales DROP COLUMN refunded_at; DELETE FROM schema_migrations WHERE version >= 60; DROP TABLE IF EXISTS venture_pipeline_upgrade", NULL, &error));
	g_assert_no_error(error);
	g_assert_true(venture_database_migrate(f->db, venture_entity_registry_get_default(), &error));
	g_assert_no_error(error);
