...
upgrade(Fixture *f, gconstpointer data)
{
	...
	money(record, "refunded", 1500);
	g_assert_true(venture_database_save(f->db, VENTURE_ENTITY(record), NULL, &error));
	/* Recreate the ordered migration suffix, including its derived pipeline
	 * queue. A prior schema has refund totals, but no reliable refund dates. */
	g_assert_true(venture_database_execute(f->db,
		"ALTER TABLE sales DROP COLUMN refunded_at; DROP TABLE IF EXISTS venture_pipeline_upgrade; DELETE FROM schema_migrations WHERE version >= 60", NULL, &error));
	g_assert_no_error(error);
	for (i = 0; i < 2; i++) {
		g_assert_true(venture_database_migrate(f->db, venture_entity_registry_get_default(), &error));
		g_assert_no_error(error);
...
module_off(Fixture *f, gconstpointer data)
{
	...
	g_assert_true(venture_database_execute(f->db, "DROP TABLE posting_profiles; DROP TABLE IF EXISTS venture_pipeline_upgrade; DELETE FROM schema_migrations WHERE version >= 60", NULL, &error));
	g_assert_true(venture_database_migrate(f->db, venture_entity_registry_get_default(), &error));
	g_assert_no_error(error);
...
upgrade_disabled(Fixture *f, gconstpointer data)
{
	...
	g_assert_true(venture_database_execute(f->db,
		"DROP TABLE posting_profiles; ALTER TABLE sales DROP COLUMN refunded_at; DROP TABLE IF EXISTS venture_pipeline_upgrade; DELETE FROM schema_migrations WHERE version >= 60", NULL, &error));
	g_assert_no_error(error);
	g_assert_true(venture_database_migrate(f->db, venture_entity_registry_get_default(), &error));
	g_assert_no_error(error);
