...
INSERT INTO venture_dunning_hardening_guard SELECT
 (SELECT COUNT(*) FROM pragma_table_info('dunning_policies') WHERE name IN ('escalation_owner', 'last_sweep')),
 (SELECT COUNT(*) FROM pragma_table_info('dunning_events') WHERE name IN ('rendered_balance_amount', 'rendered_balance_currency', 'rendered_balance_exponent')),
 (SELECT COUNT(*) FROM pragma_table_info('invoices') WHERE name IN ('dunning_paused_until', 'dunning_pause_reason')),
 (SELECT COUNT(*) FROM pragma_table_info('companies') WHERE name IN ('dunning_paused_until', 'dunning_pause_reason'));
